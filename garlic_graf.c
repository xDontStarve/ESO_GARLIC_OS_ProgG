...
void _gg_iniGrafA()
{
	...
	map2ptr = (int) bgGetMapPtr(fondo2);
		/* MapBase indica el indice del mapa, en nuestro caso cada mapa ocupa 8192Bytes, ya que en 512x512 pixeles hay
		   64x64 baldosas y cada baldosa ocupa 2Bytes en memoria para guardar la dir. de la baldosa en VRAM.
		   ...
		   Base fondo1 = 0x06000000, no se usa, usamos la dir. para guardar tiles descomprimidos.
		   Base fondo2 = 0x06002000
		   Base fondo3 = 0x06004000
		   Base fondo4 = 0x06006000, no se usa
...
void _gg_long2str(int pointed, unsigned int numPtr, char* str)
{
	...
	{
		negativo = 1;
		longNum = -longNum;						//Tratar el número en positivo, luego añadir el signo de negativo.
	}

	
	do {
		if (pointed && (digitCount > 0) && (digitCount % 3 == 0)) {
			str[i++] = '.';						//Si pointed es 1 se añaden periodos cada 3 dígitos.
		}
		str[i++] = (longNum % 10) + '0';		//Sumar '0' para poner el offset de los números en formato ASCII
	...
	while ((token[i]!='\0') && message_length<3*VCOLS)				//+1: que escribir el centinela
	{
		if (token[i]=='%')
		{
			switch (token[i+1])
			{
				...
				{
				//Si ha superado el límite de parámetros, imprimir tal cual %c
				...
				{
					int charLeft=((3*VCOLS)-message_length);
				...
				// Sumar núm de carácteres según tamaño del numero hexa
				{
					...
					//Si ha superado el límite de parámetros, imprimir tal cual %x
					//Puede que supere el límite de 3 líneas pero se ha guardado 3*VCOLS+1 espacios en la array, por lo cual el último carácter se sustituiría por \0 en escribirlinea
					{
						resultado[message_length++]='%';
						resultado[message_length++]='x';
					}
				...
				case 's':											//Copiar la string al formato final
				{
					...
					//Si ha superado el límite de parámetros, imprimir tal cual %s
					//Puede que supere el límite de 3 líneas pero se ha guardado 3*VCOLS+1 espacios en la array, por lo cual el último carácter se sustituiría por \0 en escribirlinea
					{
						resultado[message_length++]='%';
						resultado[message_length++]='s';
					}
				...
				{
					char longlong[20];	//Como máximo un long ocupa 19-20 dígitos.
					
					if (currentParamNumber == 1)
					{
						_gg_long2str(0, val1, longlong);
						currentParamNumber++;
					} else if (currentParamNumber == 2)
					{
						_gg_long2str(0, val2, longlong);
						currentParamNumber++;
					}else
					//Si ha superado el límite de parámetros, imprimir tal cual %x
					//Puede que supere el límite de 3 líneas pero se ha guardado 3*VCOLS+1 espacios en la array, por lo cual el último carácter se sustituiría por \0 en escribirlinea
				...
				// Si no cabe el número long long, se llenaré hasta que se llene el buffer
				{
					...
					if (currentParamNumber == 1)
					{
						_gg_long2str(1, val1, longlong);
						currentParamNumber++;
					} else if (currentParamNumber == 2)
					{
						_gg_long2str(1, val2, longlong);
						currentParamNumber++;
					}else
					//Si ha superado el límite de parámetros, imprimir tal cual %D
					//Puede que supere el límite de 3 líneas pero se ha guardado 3*VCOLS+1 espacios en la array, por lo cual el último carácter se sustituiría por \0 en escribirlinea
